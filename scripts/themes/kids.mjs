...
//
// 컨셈: 차가운 흰 종이 대신 **따뜻한 크림(#fff8ef)** 지면에 흰 카드를 얹고, 라즈베리
// (#c2255c)와 그레이프(#7048c6) 두 색만 강조로 쓴다. 헤더는 두 색을 잇는 그라데이션 바,
// 모서리는 전 스케일을 키워 푹신하게, 제목은 둥근 한글 서체(Jua)로 쓴다.
//
// 어린 독자를 겨냥해 색 이상으로 **읽기 밀도**를 함께 조정한다: 산문 폭을 66ch → 60ch 로
// 줄이고(짧은 줄이 눈 이동을 줄인다), 행간을 1.7/1.8 → 1.8/1.9 로 벌린다.
//
...
export default {
    root: {
        ...
        '--wiki-glass-bg': 'light-dark(rgba(255, 248, 239, 0.8), rgba(23, 19, 32, 0.8))',
